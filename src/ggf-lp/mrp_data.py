...
class MRPData:
    def __init__(self, n_group=2, n_state=3, n_action=2, weight=None):
        ...
        self.discount = 0.9
        self.transition_matrix = np.array([[[0.5, 0.5], [0.5, 0.5], [0, 0]],
                                           [[0, 0.5], [0.5, 0.5], [0.5, 0]],
                                           [[0, 0.5], [0, 0.5], [1, 0]]])
    ...
    def generate_big_cost_matrix(self):
        ...
        bigC = np.zeros([len(self.tuple_list_s), len(self.tuple_list_a), len(self.idx_list_d)])
        ...
        for s in self.idx_list_s:
            for a in self.idx_list_a:
                for d in self.idx_list_d:
                    bigC[s, a, d] = cost[self.tuple_list_s[s][d], self.tuple_list_a[a][d], d]
        return bigC
    ...
    def generate_big_transition_matrix(self):
        ...
        bigT = np.zeros([len(self.tuple_list_s), len(self.tuple_list_s), len(self.tuple_list_a)])
        ...
        for s in self.idx_list_s:
            for a in self.idx_list_a:
                for next_s in self.idx_list_s:
                    ...
                    for d in self.idx_list_d:
                        tmpT *= matrix_T[
                            self.tuple_list_s[s][d], self.tuple_list_s[next_s][d], self.tuple_list_a[a][d], d]
