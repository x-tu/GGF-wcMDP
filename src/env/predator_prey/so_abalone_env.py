...
class AbaloneParams:
    def __init__(self):
        ...
        self.init_density_by_age = np.array(
            [0.047, 0.056, 0.040, 0.023, 0.018, 0.007, 0.011, 0.003, 0.00, 0.025]
        )
        self.max_density = 3.34  # max carrying capacity (#abalone per m^2)
        self.density_by_area = np.linspace(
            0.835, 3.34, 4
        )  # density of abalone in 4 habitats
        # survival rates
        ...
        self.fertility_rates = np.array(
            [0.136, 0.26, 0.38, 0.491, 0.593, 0.683, 0.745, 0.795, 0.835, 1.166]
        )
...
class SeaOtterParams:
    def __init__(self):
        ...
        self.max_capacity_by_num = (
            4073
        )  # carrying capacity of sea otter by the total number
        ...
        self.legal_culling_rate = (
            0.6
        )  # authorise culling when sea otter density is above 60% of carrying capacity
...
class PredatorPrey(gym.Env):
    def __init__(self, out_csv_name, ggi, iFR, iFRnum, save_mode="append"):
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=self.aba.total_area * self.aba.max_density,
            shape=(11,),
            dtype=np.float32,
        )
        ...
        if save_mode not in ["append", "write"]:
            raise TypeError(
                f"{save_mode} is an invalid save mode, please choose between 'append' and 'write'"
            )
    ...
    def calculate_survival_rate_matrix(self):
        ...
        survival_matrix[0, :] = (
            self.aba.fertility_rates * self.aba.female_ratio * self.aba.sj_h
        )
    ...
    def ini_aba_pop(self):
        ...
        num_female_by_age = (
            self.aba.init_density_by_age * self.aba.female_ratio * self.aba.total_area
        )
        ...
        for _ in range(50):
            ...
            pred_rate = (
                max_r
                * best_num_female
                / (max_r * num_female - num_female + best_num_female)
            )
    ...
    def get_linear_func_response_params(self):
        ...
        """
        response_params = np.array(
            [
                [self.density_aba, self.so.max_num_aba_eaten / 3],
    ...
    def derive_hyperbolic_response(self):
        ...
        total_foraging_time = np.array(
            [0.4 * success_rate_min, 0.4 * success_rate_max, 0.4 * 1]
        )
        # total handling time
        ...
        for idx in range(len(total_foraging_time)):
            ...
            attack_rate = self.so.max_num_aba_eaten / (
                self.density_aba * total_foraging_time[idx]
        ...
        hyper_response_params = np.concatenate(
            [consumption_rates, half_saturation_consts]
        )
    ...
    def step(self, action):
        ...
        if action == 3:
            self.num_so = min(
                self.so.legal_culling_rate * self.so.max_capacity_by_num, self.num_so
            )
        if action == 4:
            num_culling = (
                self.num_so - self.so.legal_culling_rate * self.so.max_capacity_by_num
            ) / 2
    ...
    def update_abalone_population(self):
        ...
        # update the population
        self.num_aba_by_age_female = np.dot(
            self.survival_matrix, self.num_aba_by_age_female
        )
    ...
    def update_sea_otter_population(self):
        ...
        self.num_so = self.num_so * np.exp(
            self.so.growth_rate * (1 - self.num_so / self.so.max_capacity_by_num)
        )
        # oil spill happens randomly and kills a percentage of sea otters
        if np.random.rand() < self.so.oil_spill_freq:
            # random death rate
            death_rate_predict = np.random.uniform(
                self.so.death_rate_min, self.so.death_rate_max
            )
        ...
        if self.num_so != 0:
            ...
            if self.feeding_mode == 1:
                ...
                removed_aba = (
                    consumption_rate
                    * density_aba
                    / (half_saturation_const + density_aba)
                    * days
                    * self.num_so
                )
            ...
            if any(self.num_aba_by_age < 0):
                ...
                while any(self.num_aba_by_age < 0):
                    ...
                    if self.num_aba_by_age[index] < 0:
                        ...
                        index_used_to_adjust = (
                            np.mod(index + 1, len(self.num_aba_by_age)) + 1
                        )
                        # Add the negative value to the selected age group
                        self.num_aba_by_age[
                            index_used_to_adjust
                        ] -= self.num_aba_by_age[index]
    ...
    def compute_poaching_impact(self, poach_rate_factor):
        ...
        if poach_rate_factor > 0:  # High and Medium poaching
            if random_impact_level > 0.5:
                impact_factor = (
                    self.so.poach_high - self.so.poach_low
                ) * poach_rate_factor + self.so.poach_low
            else:
                impact_factor = (
                    self.so.poach_med - self.so.poach_low
                ) * poach_rate_factor + self.so.poach_low
        ...
        if out_csv_name is not None:
            if self.save_mode == "write":
                ...
                if self.run == 1:
                    ...
                    df = pd.DataFrame(self.metrics)
                    df.to_csv(
                        out_csv_name + ".csv", mode="a", header=False, index=False
                    )
            else:
