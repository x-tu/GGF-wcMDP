...
from stable_baselines.a2c_ggi.utils import Deri_GGI, total_episode_reward_logger
...
from stable_baselines.common.policies_ggi import (
    ActorCriticPolicy,
...
class PPO2(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.ini_obs_value = tf.placeholder(
                        tf.float32, [self.reward_n], name="initial_obs_value"
                    )
                    self.sor_omega = tf.placeholder(
                        tf.float32, [self.reward_n], name="self.sor_omega"
                    )
                    ...
                    clipped_pg_loss2 = tf.clip_by_value(
                        ratio, 1.0 - self.clip_range_ph, 1.0 + self.clip_range_ph
                    )
                    pg_losses2 = -self.advs_ph * tf.expand_dims(
                        clipped_pg_loss2, axis=1
                    )
    ...
    def _train_step(
        ...
        td_map = {
            ...
            self.ini_obs_value: avg_init_states,
            self.sor_omega: sorted_omega,
        }
    ...
    def learn(
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(
            ...
            for update in range(1, n_updates + 1):
                ...
                obs, avg_value, w, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward = (
                    runner.run()
                )
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        initial_state_value = self.model.value(
            np.squeeze(initial_state), self.states, self.dones
        )

        averaged_state_value = np.mean(
            initial_state_value, axis=0
        )  # average value of initial state
        ...
        sorted_omega = [
            np.where(averaged_state_value.argsort() == i)[0][0]
